...
Example:
    ...
    # 策略回测
    opentrade trade --mode paper --strategy trend_following
    
    # 实盘交易 (需要确认)
    opentrade trade --mode live --confirm
    
    # Dry-run 模式 (仅测试)
    opentrade trade --mode live --dry-run
"""
...
def main(
    ...
    confirm: bool = typer.Option(False, "--confirm", help="确认实盘交易 (live模式必须)"),
):
    """执行 AI 交易策略"""
    console = Console()
    ...
    if mode == "live" and not dry_run:
        if not confirm:
            print(Panel(
                "[bold red]⚠️ 实盘交易需要 --confirm 参数[/bold red]\n\n"
                "[yellow]请确认您已了解风险:[/yellow]\n"
                "1. 使用真实资金进行交易\n"
                "2. 可能发生亏损\n"
                "3. 无法撤销已执行的交易\n\n"
                "[green]命令:[/green] opentrade trade --mode live --confirm",
                title="🚨 实盘交易确认",
                style="red"
            ))
            raise typer.Exit(1)
